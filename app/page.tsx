...
const AGENT_ADDRESS = process.env.NEXT_PUBLIC_AGENT_ADDRESS || '0x605CEB17F2963B67C2CcCf4EF459117E4e513102';
...
export default function Home() {
  ...
  async function fetchAgentRequests() {
    ...
    try {
      const provider = new ethers.BrowserProvider((window as any).ethereum);
      const contract = new ethers.Contract(CONTRACT_ADDRESS, CONTRACT_ABI, provider);
      const ids = await contract.getRequests(AGENT_ADDRESS);
      const data = await Promise.all(ids.map(async (id: string) => {
        const req = await contract.requests(id);
        return { id, description: req.description, amount: ethers.formatUnits(req.amount, 18), paid: req.paid };
      }));
      setAgentRequests(data.reverse());
    } catch (err) { console.error(err); }
  }

  const refreshAgentRequests = async () => {
    await fetchAgentRequests();
    showToast('Agent requests refreshed!', 'success');
  };
            ...
            {activeTab === 'agent' && (
              <div className={`${cardBg} rounded-2xl p-6 border ${borderClass}`}>
                ...
                <div className="flex justify-end mb-3">
                  <button onClick={refreshAgentRequests} className="text-xs bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-lg transition flex items-center gap-1">
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-6">
                  ...
                  <div className="bg-white/5 rounded-xl p-4">
                    ...
                    <p className="text-sm text-gray-400">Address: <span className="text-xs font-mono">{AGENT_ADDRESS.slice(0,10)}...{AGENT_ADDRESS.slice(-6)}</span></p>
                    <p className="text-sm text-gray-400">Status: <span className="text-green-400">Active</span></p>
                    <p className="text-xs text-gray-400 mt-2">Balance: ~12.5 USDC (testnet)</p>
                  </div>
                </div>
                <div className="bg-white/5 rounded-xl p-4 mb-6">
                  <h3 className="font-semibold mb-3">How it works</h3>
                  <div className="text-sm text-gray-400 space-y-2">
                    <p>1. Agent calls <code className="bg-black/30 px-1 rounded">GET /api/agent</code> with description and amount</p>
                    ...
                    <p>5. Check payment status via <code className="bg-black/30 px-1 rounded">POST /api/agent/pay</code></p>
                ...
                <div className="bg-white/5 rounded-xl p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-2 text-xs text-gray-400 border-b border-white/10 pb-2">
                    <span>Request</span>
                    <span>Amount</span>
                    <span className="text-right">Status</span>
                  </div>
                  {agentRequests.length === 0 ? (
                    <p className="text-sm text-gray-400 text-center py-4">No agent requests yet</p>
                  ) : (
                    <div className="space-y-2">
                      {agentRequests.map((req, idx) => (
                        <div key={idx} className="grid grid-cols-3 gap-2 items-center border-b border-white/5 py-2 text-sm">
                          <div className="truncate">
                            <p className="font-medium text-xs">{req.description}</p>
                            <p className="text-[10px] text-gray-400 font-mono">{req.id.slice(0,8)}...</p>
                          </div>
                          <p className="font-bold text-xs">{req.amount} USDC</p>
                          <p className={`text-xs text-right font-medium ${req.paid ? 'text-green-400' : 'text-yellow-400'}`}>
