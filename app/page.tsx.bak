          ...
          <div className="flex items-center gap-3">
            ...
            {!wallet ? (
              ...
              <div className="flex items-center gap-2 md:gap-3 bg-white/5 rounded-full border border-white/10 px-2 py-1 md:px-3 md:py-1.5">
                <div className="flex items-center gap-2"><div className="w-2 h-2 bg-green-400 rounded-full animate-pulse shadow-lg shadow-green-400/50"></div><span className="font-mono text-sm">{wallet.slice(0,6)}...{wallet.slice(-4)}</span><span className="text-xs bg-gradient-to-r from-pink-500/30 to-cyan-500/30 px-2 py-0.5 rounded-full flex items-center gap-1">{parseFloat(balance).toFixed(2)} USDC<button onClick={fetchBalance} className="hover:rotate-180 transition" title="Refresh">🔄</button></span><span className="hidden md:inline-flex text-xs bg-yellow-500/30 px-2 py-0.5 rounded-full items-center gap-1"><Award className="w-3 h-3" /> {totalBadges}</span><span className="hidden md:inline-flex text-xs bg-purple-500/30 px-2 py-0.5 rounded-full items-center gap-1"><Star className="w-3 h-3" /> {userPoints}</span></div>
                <button onClick={disconnectWallet} className="text-gray-300 hover:text-white px-3 py-1.5 rounded-lg hover:bg-white/10 transition text-base font-medium">🔌</button>
              </div>
            )}
          </div>
        </div>
